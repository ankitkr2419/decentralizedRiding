...
class RiderPage extends React.Component {
  ...
  onRequestSubmit(event) {
    ...
    if (this.props.DEV) {
      ...
      endLoc = { lat: 40.3421, lng: 100.335 };
  ...
  onRouteMade(payload) {
    ...
    const totalDistance = metersToMiles(
      totalRouteDistance(directions.routes[0])
    );
  ...
  render() {
    ...
    const primaryButton = {
      submitFunction: this.onRequestSubmit,
      label: "Request",
    };
    const secondaryButton = {
      submitFunction: this.onCancelSubmit,
      label: "Cancel",
    };
    const directions =
      this.state.startLocation && this.state.endLocation
        ? { startLoc: this.state.startLocation, endLoc: this.state.endLocation }
        : null;
