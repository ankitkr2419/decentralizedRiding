...
class DriverPage extends React.Component {
  ...
  async getArrivalTime(rideInfo) {
    ...
    return [directions.routes[0].legs[0].duration.value, startLoc, endLoc];
  }
  ...
  async onJobAccept(event) {
    ...
    for (let ride of this.state.currentRides) {
      if (
        ride.startAddr == jobInfo[0] ||
        (ride.startAddr == jobInfo[1] && ride.endAddr == jobInfo[0]) ||
        ride.endAddr == jobInfo[1]
      ) {
        rideInfo = ride;
      }
    }
  ...
  async getRides(payload) {
    ...
    const makeForDistanceCalc = (loc) => {
      return {
        lat: () => loc.lat / multFactor,
        lng: () => loc.long / multFactor,
      };
    };
    ...
    let nonCallableStartLoc = {
      lat: callableStartLoc.lat(),
      lng: callableStartLoc.lng(),
    };
    let nonCallableEndLoc = {
      lat: callableEndLoc.lat(),
      lng: callableEndLoc.lng(),
    };
  ...
  renderJobButtons() {
    if (
      !this.props.DEV &&
      (!this.state.currentRides || !this.state.currentRides.length)
    ) {
      return <div></div>;
    }
  ...
  render() {
    ...
    // look for new rides
    if (
      !this.localVals.requestedRides &&
      this.props.getAvailableRidesListener
    ) {
