...
class RiderProgressPage extends React.Component {
  ...
  tripEnded(payload) {
    ...
    if (
      payload &&
      payload.returnValues.pairNumber &&
      (payload.returnValues.pairNumber == this.props.riderNumber ||
        (payload.returnValues.usrNumber == this.props.riderNumber &&
          this.state.paid == this.props.tripRate))
    ) {
      let driverCancelled =
        this.state.paid == this.props.tripRate ? false : true;
      const cancelFee = payload.returnValues.cancelFee
        ? payload.returnValues.cancelFee
        : 0;
      this.setState({
        tripEnded: true,
        driverCancelled,
  ...
  cancelTrip() {
    ...
    if (this.state.rideAccepted) {
      ...
      if (
        window.confirm(
          "Are you sure you want to cancel? You will lose any money paid to the driver so far."
        )
      ) {
      ...
      if (
        window.confirm(
          "Are you sure you want to cancel? You will not be penalized for cancelling at this time."
        )
      ) {
        this.props.cancelTrip();
      }
  ...
  onRideAccepted(payload) {
    ...
    if (this.props.riderNumber != incomingRiderNumber) {
      console.log(
        "Incoming rider number " +
          incomingRiderNumber +
          " does not match my rider number " +
          this.props.riderNumber
      );
      return;
    }
  ...
  mockTrip() {
    ...
    for (let leg of route.steps) {
      const sleepTime = Math.round(
        metersToMiles(leg.distance.value) * mockTimePerMile * 1000
      ); // ms to seconds
        ...
        <div className="summaryContainer">
          {this.state.driverCancelled && (
            <div className="driverCancelledContainer">
              ...
              <p>
                Fee recieved from cancellation: {this.state.feeFromDriverCancel}
              </p>
            </div>
          )}
          {!this.state.driverCancelled && (
  ...
  renderWaitingProgressBar() {
    ...
    const totalTime = Math.round(
      this.state.timeToDriverArrival - this.state.startTime
    );
    ...
    return (
      <div className="progressBarContainer">
        <div>
          <b>Time till driver arrives:</b>
        </div>
        {/* <ProgressBar
          animated
          variant="warning"
          now={percent}
          label={`${this.state.remainingTime}s`}
        ></ProgressBar> */}
        <ProgressBar
          color="success"
      ...
      <div className="RiderProgressPage">
        ...
        <div className="RiderDetails">
          <div className="Details">
            <span className="DetailContainer" style={{ display: "block" }}>
              <span className="DetailLabel" id="PickupDetails">
                Pick Up Location:{" "}
              </span>
              {this.props.riderPickupAddress}
            </span>
            <span className="DetailContainer">
              <span className="DetailLabel" id="DropOffDetails">
                Drop Off Location:{" "}
              </span>
              {this.props.riderDropOffAddress}
            </span>
          </div>
        </div>
        <div className="RiderProgressPageCenter">
          <table>
            <tbody>
              <tr>
                <td>{this.renderSummary()}</td>
              </tr>
              <tr>
                <td>{this.renderSpinner()}</td>
              </tr>
              <tr>
