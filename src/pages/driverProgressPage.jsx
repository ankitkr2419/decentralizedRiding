...
class DriverProgressPage extends React.Component {
  ...
  tripEnded(payload) {
    if (
      ...
      payload.returnValues.pairNumber != this.props.driverNumber
    ) {
      return;
    }
  ...
  cancelTrip(payload) {
    ...
    if (payload && payload.fromTimeOut) {
      ...
      if (
        window.confirm(
          "Are you sure you want to cancel? The rider will stop paying you if you do."
        )
      ) {
        this.resetState();
        this.props.cancelTrip();
      ...
      if (
        window.confirm(
          "Are you sure you want to cancel? You will lose your deposit."
        )
      ) {
        this.resetState();
        this.props.cancelTrip();
  ...
  onDriverPaid(payload) {
    ...
    this.setState((prevState) => ({
      paid: (prevState.paid += nextPayment),
      tripEnded,
    }));
  }
      ...
      return (
        <div className="summaryContainer">
          {!this.state.riderCancelled && (
            <div className="rideCompleteName">Ride Complete!</div>
          )}
          {this.state.riderCancelled && (
            <div className="riderCancelledContainer">Rider cancelled</div>
          )}
          <div className="amountPaid">Amount Recieved: {this.state.paid}</div>
      ...
      <div>
        ...
        {this.state.remainingTime <= 60 && (
          <div className="timeoutWarning">
            You have {this.state.remainingTime}s to arrive or you will be
            penalized and returned to the home page
          </div>
        )}
      </div>
    );
