...
async function initBlockchain(portNumber, contractAddress, abiInterface) {
  web3 = new Web3(
    new Web3.providers.WebsocketProvider("ws://localhost:" + portNumber)
  );
...
function requestRide(startLoc, endLoc, rideCost, ethereumAddress) {
  ...
  return new Promise((resolve, reject) => {
    contract.methods
      .rideRequest(startLatLng, endLatLng, rideCost)
      .send({ from: ethereumAddress, gas: gasLimit })
      .then((value) => {
        console.log(value);
        resolve(value);
...
function acceptJob(riderNumber, timeToArrive, ethereumAddress) {
  ...
  contract.methods
    .pickRider(riderNumber, timeRemaining)
    .send({ from: ethereumAddress, gas: gasLimit })
    .then((value) => {
      console.log(value);
    });
...
function getCurrentRides(ethereumAddress) {
  contract.methods
    ...
    .catch((error) =>
      console.log(
        "ERROR: CANNOT GET WAITING RIDERS\n" + error + "\nGAS AMOUNT" + gasLimit
      )
    );
}
  ...
  return new Promise((resolve, reject) => {
    contract.methods
      ...
      .send({
        ...
        gas: gasLimit,
        value: web3.utils.toWei(feeAmount, "wei"),
      })
      .then((value) => {
...
function informRider(loc, ethereumAddress) {
  ...
  contract.methods
    .informRider(locLatLng)
    .send({ from: ethereumAddress, gas: gasLimit })
    .then((val) => {
      console.log("Sent inform rider message");
    })
    .catch((error) =>
      console.log("Error: not close enough to the rider. Please move closer")
    );
}
