...
class App extends Component {
  ...
  async onPageRefresh() {
    ...
    alert(
      "You have lost " +
        cancelFee +
        " amount of money for cancelling your trip."
    );
  }

  async loadBlockchainData() {
    if (NO_BLOCKCHAIN_DEV) {
      return;
    }
    ...
    const address = "0x5b3BcbdC463Bcda3AbB11548ed9a828EAd471C9B";

    const blockchainFunctions = await initBlockchain(
      portNumber,
      address,
      derisInterface
    );
    const getAvailableRidesListener = (cb) =>
      blockchainFunctions.events
        .RiderDetails({})
        .on("data", (event) => cb(event));
    const onRideAcceptedListener = (cb) =>
      blockchainFunctions.events
        .RiderPicked({})
        .on("data", (event) => cb(event));
    const onDriverArrivedListener = (cb) =>
      blockchainFunctions.events.imHere({}).on("data", (event) => cb(event));
    const onDriverPaidListener = (cb) =>
      blockchainFunctions.events.cashMoney({}).on("data", (event) => cb(event));
    const onTripEndedListener = (cb) =>
      blockchainFunctions.events.undone({}).on("data", (event) => cb(event));

    this.setState({
      blockchainFunctions,
      onRideAcceptedListener,
      getAvailableRidesListener,
      onDriverArrivedListener,
      onDriverPaidListener,
      onTripEndedListener,
    });
  }
  ...
  async onRiderPageSubmit(payload) {
    ...
    if (isContinuing && !NO_BLOCKCHAIN_DEV) {
      const startLoc = {
        lat: payload.startLocation.lat().toString(),
        lng: payload.startLocation.lng().toString(),
      };
      const endLoc = {
        lat: payload.endLocation.lat(),
        lng: payload.endLocation.lng(),
      };
  ...
  async onDriverPageSubmit(payload) {
    ...
    if (isCancel) {
      ...
      acceptJob(
        parseInt(payload.jobInfo.riderNumber),
        payload.arrivalTime + FORGIVENESS_TIME,
